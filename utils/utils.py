...
# @Author : system
# @Version : 0.2.5
# @Program : 常用方法 工具类 | Common method Tool class
# @File : utils.py
# @Description : 定义 常用方法 工具类 | Definition Common method Tool class
...
def sort_two_list(list1, list2, reverse=False):
    ...
    """
    list1, list2 = (list(t)
                    for t in zip(*sorted(zip(list1, list2), reverse=reverse)))
    return list1, list2
    ...
    try:
        if is_delete:
            if os.path.exists(dir_name):
                ...
                print('[Info] folder "%s" presence, Delete folder.' % dir_name)

        if not os.path.exists(dir_name):
            os.makedirs(dir_name)
            print(
                '[Info] folder "%s" does not exist, Create folder.' %
                dir_name)
        return True
...
def create_folder(atp_out_dir):
    ...
    if os.path.exists(atp_out_dir):
        ...
        print('folder "%s" presence，Delete folder。' % atp_out_dir)

    if not os.path.exists(atp_out_dir):
        os.makedirs(atp_out_dir)
        print('folder "%s" does not exist，Create folder。' % atp_out_dir)
...
def create_file(file_name):
    ...
    if os.path.exists(file_name):
        print("File exists, delete file：%s" % file_name)
        os.remove(file_name)  # 删除已有文件
    if not os.path.exists(file_name):
        print("File does not exist, create file：%s" % file_name)
...
def write_line_utf8(file_name, line):
    ...
    with io.open(file_name, "a+", encoding='utf8') as fs:
        if isinstance(line, (tuple or list)):
            fs.write("%s\n" % ", ".join(line))
        else:
            fs.write("%s\n" % line)
