# -*- coding:utf-8 -*-
#
# Geek International Park | 极客国际公园 & GeekParkHub | 极客实验室
# Website | https://www.geekparkhub.com
# Description | Open · Creation |
# Open Source Open Achievement Dream, GeekParkHub Co-construction has never been seen before.
#
# HackerParkHub | 黑客公园
# Website | https://www.hackerparkhub.org
# Description | In the spirit of fearless exploration, create unknown technology and worship of technology.
#
# AIParkHub | 人工智能公园
# Website | https://github.com/aiparkhub
# Description | Embark on the wave of AI and push the limits of machine intelligence.
#
# @GeekDeveloper : JEEP-711
# @Author : system
# @Version : 0.2.5
# @Program : 初始化 自定义 (网络结构示例) | Initialization Custom (network structure example)
# @File : __init__.py
# @Description : 定义 初始化 自定义 (网络结构示例) | Definition Initialization Custom (network structure example)
# @Copyright © 2019 - 2020 AIParkHub-Group. All rights reserved.


__author__ = """system"""
__email__ = 'jeep-711@outlook.com'
__version__ = '0.2.5'
